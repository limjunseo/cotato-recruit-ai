...
2) 면접질문은 위 "자기소개서 질문"과 "자기소개서 답변"을 함께 참고해 0~${questionCount}개 작성하세요.
3) 각 질문은 "자기소개서 질문"의 주제를 유지하면서 "자기소개서 답변"의 사례를 검증해야 합니다.
4) "자기소개서 질문" 재진술 금지, "자기소개서 답변"에 없는 사실 단정 금지.
5) 답변이 너무 짧거나 근거가 부족하면 "questions"를 빈 배열([])로 두어도 됩니다.
6) JSON 객체만 출력하고, 코드블록/마크다운/설명문/접두어는 금지합니다.

출력 스키마(JSON, questions는 빈 배열 허용):
{
...
4) "questions"는 위 "자기소개서 질문"과 "자기소개서 답변"을 함께 참고해 0~${questionCount}개 작성
5) "자기소개서 질문" 재진술 금지, "자기소개서 답변"에 없는 사실 단정 금지
6) 답변 근거가 부족하면 "questions"를 빈 배열([])로 둘 수 있음
7) 코드블록/마크다운/설명문 금지
